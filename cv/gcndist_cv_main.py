...
from graph_input import MultiTaskCalsDataset, MultiTaskCalsListDataset
...
parser.add_argument('--model_path', metavar='model_path',
                    default='../models/baseline_cv/gat/checkpoint.pth.tar', type=str)
...
def main():
    ...
    for fold, (train_index, test_index) in enumerate(skf.split(first_ids, first_y)):
        if fold==run_fold:
            wandb.init(project='multi-task-baseline', name=f'GATdist3_fold_{fold + 1}')
            ...
            # Create model
            model = gatconv_dist(num_features=args.num_features,
            ...
            for epoch in range(args.start_epoch, args.num_epochs):
                ...
                if (epoch+1) % 600 == 0:
                    ...
                    }, is_best=0, filename=os.path.join(os.path.dirname(args.model_path),
                                                        str(fold),
                                                        'distribution/checkpoint.pth.tar'))
